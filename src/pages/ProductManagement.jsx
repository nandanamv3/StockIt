...
function Products() {
  ...
  const [user, setUser] = useState(null); // 1. Add state for the user
  ...
  const logInventoryChange = async (productId, changeType, quantityChanged, productName, sku) => {
    if (quantityChanged === 0) return; // Skip logging if no quantity change
    const { error } = await supabase
      .from('inventory_logs')
      .insert([{
        product_id: productId,
        user_id: user?.id, // Also add user_id to logs
  ...
  const handleSubmit = async (e) => {
    ...
    // Add a check to ensure user is loaded
    if (!user) {
    ...
    let imageUrl = null;
    if (image_file) {
      const fileName = `${Date.now()}_${image_file.name}`; // Unique filename
    ...
    try {
      if (isEditing) {
        // ... (No change needed for the editing logic)
...
      image_file: null, // File input starts empty
